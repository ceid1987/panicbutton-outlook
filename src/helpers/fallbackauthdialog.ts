...
const loginRequest: RedirectRequest = {
  scopes: [accessScope],
  extraScopesToConsent: ["user.read", "mail.read", "mail.readwrite", "mail.send"],
};
...
export async function dialogForwardFallback(messageId, forwardToAddress, callback) {
  // Attempt to acquire token silently if user is already signed in.
  if (homeAccountId !== null) {
    const result = await publicClientApp.acquireTokenSilent(loginRequest);
    if (result !== null && result.accessToken !== null) {
      const response = await callForwardAndDelete(result.accessToken, messageId, forwardToAddress);
      callback(response);
    }
  } else {
    callbackFunction = async (result) => {
      const response = await callForwardAndDelete(result, messageId, forwardToAddress);
      callback(response);
    };
...
async function processMessage(arg) {
  ...
  if (messageFromDialog.status === "success") {
    ...
    const response = await callbackFunction(messageFromDialog.result);
    showMessage(response.message || "Action completed successfully.");
    callbackFunction(response);
