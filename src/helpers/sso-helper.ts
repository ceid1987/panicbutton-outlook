...
import { dialogForwardFallback } from "./fallbackauthdialog";
import { callGetUserData } from "./middle-tier-calls";
import { callForwardAndDelete } from "./middle-tier-calls";
// import { callForwardAndDelete } from "./middle-tier-calls";
...
export async function forwardAndDelete(messageId, forwardToAddress, callback): Promise<void> {
  try {
    // 1 - Get the middle tier token for MS Graph Access
    ...
    // 2 - Perform the forward and delete operation
    let response: any = await callForwardAndDelete(middletierToken, messageId, forwardToAddress);
    if (!response) {
      throw new Error("Middle tier didn't respond");
    } else if (response.claims) {
      // Microsoft Graph requires an additional form of authentication
      let mfaMiddletierToken: string = await OfficeRuntime.auth.getAccessToken({
        authChallenge: response.claims,
      });
      response = callForwardAndDelete(mfaMiddletierToken, messageId, forwardToAddress);
    }

    // AAD
    if (response.error) {
      handleForwardAADErrors(response, messageId, forwardToAddress, callback);
    }
  } catch (exception) {
    if (exception.code) {
      if (handleClientSideErrors(exception)) {
        dialogForwardFallback(messageId, forwardToAddress, callback);
      }
    } else {
...
function handleForwardAADErrors(response: any, messageId: string, forwardToAddress: string, callback: any): void {
  // On rare occasions the middle tier token is unexpired when Office validates it,
  // but expires by the time it is sent to AAD for exchange. AAD will respond
  // with "The provided value for the 'assertion' is not valid. The assertion has expired."
  // Retry the call of getAccessToken (no more than once). This time Office will return a
  ...
  if (response.error_description.indexOf("AADSTS500133") !== -1 && retryGetMiddletierToken <= 0) {
    retryGetMiddletierToken++;
    forwardAndDelete(messageId, forwardToAddress, callback);
  } else {
    dialogForwardFallback(messageId, forwardToAddress, callback);
  }
}
