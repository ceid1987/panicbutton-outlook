...
export async function forwardAndDelete(req: any, res: any, next: any) {
  const authorization: string = req.get("Authorization");
  const { messageId, forwardToAddress } = req.body;
  ...
  await getAccessToken(authorization)
    .then(async (graphTokenResponse) => {
      if (graphTokenResponse && (graphTokenResponse.claims || graphTokenResponse.error)) {
        ...
        const forwardResponse = await performForwardOperation(graphToken, messageId, forwardToAddress);
        if (forwardResponse.code) {
          return next(createError(forwardResponse.code, "MS Graph Error" + JSON.stringify(forwardResponse)));
        }

        // If forwarding is successful, delete the message
        try {
          const deleteResponse = await performDeleteOperation(graphToken, messageId);
          if (deleteResponse && deleteResponse.code) {
            return next(createError(deleteResponse.code, "MS Graph Error " + JSON.stringify(deleteResponse)));
          } else {
            res.send({ status: "Success", message: "Message forwarded and deleted successfully." });
          }
        } catch (error) {
          return next(error);
        }
...
async function performForwardOperation(accessToken: string, messageId: string, forwardToAddress: string): Promise<any> {
  const forwardApiUrl = `/me/messages/${messageId}/forward`;
  const forwardBody = JSON.stringify({
    Comment: "Forwarded by Panic Button",
    ToRecipients: [
      {
        EmailAddress: {
    ...
    const req = https.request(options, (response) => {
      ...
      response.on("end", () => {
        ...
        if (response.statusCode === 202 || response.statusCode === 204) {
          resolve({ status: "Success", message: "Message forwarded successfully." });
        } else {
          error = new Error();
          error.code = response.statusCode;
          error.message = response.statusMessage;
...
async function performDeleteOperation(accessToken: string, messageId: string): Promise<any> {
  const deleteApiUrl = `/me/messages/${messageId}`;

  return new Promise<any>((resolve, reject) => {
    const options: https.RequestOptions = {
      host: domain,
      path: "/" + version + deleteApiUrl,
      method: "DELETE",
      headers: {
        Authorization: "Bearer " + accessToken,
      },
    ...
    const req = https.request(options, (response) => {
      response.on("end", () => {
        if (response.statusCode === 204) {
          resolve({ status: "Success", message: "Message deleted successfully." });
        } else {
          reject(new Error(`Failed to delete message. Status code: ${response.statusCode}`));
        }
      });
    });

    req.on("error", reject);
    req.end(); // Ensure no body is sent with the request
  });
}
